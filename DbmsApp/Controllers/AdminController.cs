...
public class AdminController : Controller
{
	...
	{
		_db.Database.ExecuteSqlRaw($"EXEC UpdateUserRole @userId={id}, @role = 'USR'");
		_db.SaveChanges();
	...
	{
        _db.Database.ExecuteSqlRaw($"EXEC UpdateUserRole @userId={id}, @role = 'STF'");
        _db.SaveChanges();
		return RedirectToAction("Users");
	}
	...
	{
        _db.Database.ExecuteSqlRaw($"EXEC UpdateUserRole @userId={id}, @role = 'ADM'");
        _db.SaveChanges();
		return RedirectToAction("Users");
	}
	...
	public IActionResult Coupons(Coupon coup)
	{
        ...
        _db.Database.ExecuteSqlRaw(
            $"INSERT INTO Logs([log]) VALUES (FORMAT(GETDATE(), 'yyyy-MM-dd HH:mm:ss') + N' - New coupon #{coup.Number} was created')");
	...
	{
		Console.WriteLine($"INSERT INTO GoodsToCoupons(couponId, goodsId, count) " +
						$"VALUES ({id}, {good}, {count})");
		_db.Database.ExecuteSqlRaw($"INSERT INTO GoodsToCoupons(couponId, goodsId, count) " +
									$"VALUES ({id}, {good}, {count})");
		_db.SaveChanges();
	...
	{
		var coup = _db.Coupons.FromSqlRaw($"SELECT * FROM Coupons WHERE id={id}").First();
        ...
        _db.Database.ExecuteSqlRaw(
    $"INSERT INTO Logs([log]) VALUES (FORMAT(GETDATE(), 'yyyy-MM-dd HH:mm:ss') + N' - Coupon #{coup.Number} was deleted')");
        _db.SaveChanges();
		return RedirectToAction("Coupons");
	}
}