...
public class CartController : Controller
{
	...
	public IActionResult Index(string address, string? entrance, string? number)
	{
		...
		_db.Database.ExecuteSqlRaw(
			...
			$"VALUES(N'{addr.Adress}', '{addr.Entrance}', '{addr.Number}', {_us.CurrentUser.Id})");
		_db.SaveChanges();
		...
		foreach ((var goodId, var count) in _us.GoodsInCart)
		{
			...
			_db.Database.ExecuteSqlRaw($"INSERT INTO GoodsToOrders(orderId, goodsId, [count]) " +
